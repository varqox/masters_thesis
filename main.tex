...
The client has to spawn the sandbox server --- the supervisor process.

\begin{lstlisting}[language=C++, title=spawn\_supervisor(),numbers=left,belowskip=0pt,xleftmargin=12pt,basicstyle=\small\ttfamily,showstringspaces=false,numbersep=5pt,numberstyle=\small\ttfamily,columns=spaceflexible,keepspaces=true,tabsize=4,breaklines=true,commentstyle=\color{gray},keywordstyle=\color{blue}\bfseries,stringstyle=\color{purple}]
SupervisorConnection spawn_supervisor();
\end{lstlisting}

Through \texttt{SupervisorConnection} object the client has access to requesting secure execution of programs.

\begin{lstlisting}[language=C++, title=SupervisorConnection class,numbers=left,belowskip=0pt,xleftmargin=12pt,basicstyle=\small\ttfamily,showstringspaces=false,numbersep=5pt,numberstyle=\small\ttfamily,columns=spaceflexible,keepspaces=true,tabsize=4,breaklines=true,commentstyle=\color{gray},keywordstyle=\color{blue}\bfseries,stringstyle=\color{purple}]
...
public:
    ...
    class RequestHandle;
    class KillRequestHandle;

    /// Sends the request and returns immediately without waiting for the request
    /// to complete. Multiple requests can be send before awaiting completion
    /// with await_result(). Throws if there is any error with the supervisor.
    [[nodiscard]] RequestHandle
    send_request(int executable_fd, Slice<std::string_view> argv,
    ...
    /// with await_result(). Throws if there is any error with the supervisor.
    [[nodiscard]] RequestHandle
    send_request(std::string_view executable_path, Slice<std::string_view> argv,
                 const RequestOptions&);
    ...
    /// with await_result(). Throws if there is any error with the supervisor.
    [[nodiscard]] RequestHandle
    send_request(Slice<std::string_view> argv,
...
\end{lstlisting}
\texttt{send\_request()} returns \texttt{RequestHandle} object that can be used to obtain result of the execution, cancel execution or get the \texttt{KillRequestHandle}. Canceling execution is useful in case of errors in the client, where result of the execution as well as the execution itself is no longer needed. Tracee processes if they are executing are immediately killed and the result is discarded. Canceling an already finished request discards the result. Canceling the request before the server started handling it causes it to be skipped.
\begin{lstlisting}[language=C++, title=SupervisorConnection::RequestHandle class,numbers=left,belowskip=0pt,xleftmargin=12pt,basicstyle=\small\ttfamily,showstringspaces=false,numbersep=5pt,numberstyle=\small\ttfamily,columns=spaceflexible,keepspaces=true,tabsize=4,breaklines=true,commentstyle=\color{gray},keywordstyle=\color{blue}\bfseries,stringstyle=\color{purple}]
class SupervisorConnection {
public:
    ...
    public:
        ...
        [[nodiscard]] int pollable_fd() const noexcept;
        // can be called only once
        KillRequestHandle get_kill_handle() noexcept;
    };
};
\end{lstlisting}
The \texttt{KillRequestHandle} object allows killing all tracee processes. Killing an already finished request is no-op. Killing the request before the server started handling it causes tracee to be immediately killed after \texttt{execveat}.
\begin{lstlisting}[language=C++, title=SupervisorConnection::KillRequestHandle class,numbers=left,belowskip=0pt,xleftmargin=12pt,basicstyle=\small\ttfamily,showstringspaces=false,numbersep=5pt,numberstyle=\small\ttfamily,columns=spaceflexible,keepspaces=true,tabsize=4,breaklines=true,commentstyle=\color{gray},keywordstyle=\color{blue}\bfseries,stringstyle=\color{purple}]
class SupervisorConnection {
public:
...
\end{lstlisting}
\texttt{await\_result()} returns result of the execution of the request. It can either be an error with textual description. This error is not fatal to the supervisor process i.e. new requests can be sent. The result consist of the exit's code and status, and runtime statistics:
\begin{itemize}
    \item real time aka \texttt{result::Ok::runtime},
    \item CPU user and CPU system time aka \texttt{result::Ok::cgroup::cpu\_time::user} and \\\texttt{result::Ok::cgroup::cpu\_time::system},
    \item peak tracee cgroup memory usage aka \texttt{result::Ok::cgroup::peak\_memory\_in\_bytes}.
\end{itemize}
\begin{lstlisting}[language=C++, title=SupervisorConnection::await\_result() return type,numbers=left,belowskip=0pt,xleftmargin=12pt,basicstyle=\small\ttfamily,showstringspaces=false,numbersep=5pt,numberstyle=\small\ttfamily,columns=spaceflexible,keepspaces=true,tabsize=4,breaklines=true,commentstyle=\color{gray},keywordstyle=\color{blue}\bfseries,stringstyle=\color{purple}]
...
struct Ok {
    ...
    struct Cgroup {
        struct CpuTime {
            ...
            [[nodiscard]] std::chrono::microseconds total() const noexcept {
                return user + system;
            }
        } cpu_time;
...
\end{lstlisting}
The \texttt{RequestOptions} allows specifying:
\begin{itemize}
    \item \texttt{stdin}, \texttt{stdout}, and \texttt{stderr} file descriptors. If \texttt{std::nullopt} is specified \texttt{/dev/null} is opened as the file descriptor.
...
struct RequestOptions {
    ...
    struct LinuxNamespaces {
        struct User {
            // nullopt will leave it the same as the outside effective
            // user ID (euid)
            std::optional<uid_t> inside_uid = std::nullopt;
            // nullopt will leave it the same as the outside effective
            // group ID (egid)
            std::optional<gid_t> inside_gid = std::nullopt;
        } user = {};

        struct Mount {
            struct MountTmpfs {
                std::string_view path;
                // Will be rounded up to page size multiple; 0 rounds up to
                // page size due to technical limitations.
                ...
                // Do not allow programs to be executed on this mount
                bool no_exec = true;
            };
            ...
            struct BindMount {
                // Path to file or directory to bind mount
                ...
                // Do not allow programs to be executed on this mount
                bool no_exec = true;
                // Do not follow symbolic links if source points to one
            ...
            using Operation = std::variant<
                MountTmpfs, MountProc, BindMount, CreateDir, CreateFile
            >;
    ...
    struct Prlimit {
        // RLIMIT_AS
        std::optional<uint64_t> max_address_space_size_in_bytes = std::nullopt;
        // RLIMIT_CORE
        std::optional<uint64_t> max_core_file_size_in_bytes = std::nullopt;
        // RLIMIT_CPU
        std::optional<uint64_t> cpu_time_limit_in_seconds = std::nullopt;
        // RLIMIT_FSIZE
        std::optional<uint64_t> max_file_size_in_bytes = std::nullopt;
        // RLIMIT_NOFILE
        std::optional<uint64_t> file_descriptors_num_limit = std::nullopt;
        // RLIMIT_STACK
        std::optional<uint64_t> max_stack_size_in_bytes = std::nullopt;
    } prlimit = {};
